...
export default function MissionVision() {
  return (
    <section className="bg-[#0A1A2F]" style={{ padding: '96px 0' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: '32px' }}>
          {cards.map((c, i) => (
            <motion.div
              ...
              className={`rounded-[20px] border ${c.border} bg-[#0d2240]/50 backdrop-blur-sm`}
              style={{ padding: '32px', boxShadow: `0 0 40px ${c.glow}` }}
            >
              <div className="text-3xl mb-4">{c.icon}</div>
              <p className="font-montserrat text-sky-400 text-xs tracking-[0.4em] uppercase mb-3">
                {c.label}
              </p>
              <h3 className="font-bebas text-4xl text-white tracking-wider mb-4">{c.title}</h3>
